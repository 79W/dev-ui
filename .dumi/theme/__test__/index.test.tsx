...
describe('mobile theme', () => {
  ...
  beforeAll(() => {
    ...
    window.HTMLElement.prototype.getBoundingClientRect = () => {
      const top =
        document.querySelector('.__dumi-default-mobile-previewer') === this
          ? 130
          : 200
  ...
  it('should render builtin components correctly', async () => {
    ...
    const routeUpdateDefer = new Promise<void>(resolve => {
      ;(
        getByTitle('dumi-previewer') as HTMLIFrameElement
    ...
    await act(async () => {
      ...
      document.documentElement.scrollTop +=
        secondDemo.getBoundingClientRect().top + 1
  ...
  it('should render device with carrier', async () => {
    ...
    expect(
      document.querySelector('.__dumi-default-device-status-carrier').innerHTML
    ).toEqual('test carrier')
  })
  ...
  it('should render without simulator', async () => {
    ...
    expect(
      document.querySelector('.__dumi-default-device').textContent
    ).toContain("I'm a!")
  })
})
